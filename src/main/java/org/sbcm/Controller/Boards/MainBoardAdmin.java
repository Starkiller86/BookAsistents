...
public class MainBoardAdmin implements Initializable {
...
//Primero vamos a crear el objeto en el que vamos a colocar toda la info de la interfaz
    ...
    public void deleteAction(ActionEvent event) {
        ...
        try{
            ...
            Adult adult = tableList.getSelectionModel().getSelectedItem();
            if (adult == null){
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Error");
            ...
            assert adult != null;
                ...
                alert.setContentText("Desea eliminar al registro con el id: " + adult.getId());
                alert.showAndWait().ifPresent(response ->{
                    if (response == ButtonType.OK)
                    {
                        ...
                        try {
                            adultCRUD.deleteResource(adult.getId());
                        } catch (Exception e) {
                            Alert alert1 = new Alert(Alert.AlertType.ERROR);
                            alert1.setHeaderText("Error de conexión con el servidor");
                            throw new RuntimeException(e);
                        }
                    }
                    else {
